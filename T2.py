...
#st_year,ed_year,more,dpath1,dpath2 = 2000,2001,3,'/raid63/pfzhou/WRF_reinit_GLDASsnow_SKINsst_SN/','/raid61/WRF_reinit_GLDASsnow_SKINsst/Output'
...
for year in range(st_year,ed_year,1):
    ...
    T2 = root.createVariable('T2',np.float32,('time','lat','lon'))
    #SWDOWN,GLW
    # 对nc文件进行描述
    ...
    #                print(dfile)
            ...
            T2[count:count+len(dfiles)-1] = data_reinit_sn.T2[1:len(dfiles)].values
        else :
            raise NameError
        count += 24
    root.close()
    ...
    T2[:len(dates)] = np.average(T2_hourly,axis = 1)
    ...
    for i in range(12): 
        times[i] = i+1
        T2[i] = np.average(T2_daily[index_month[i]:index_month[i+1]],axis = 0)
    root.close()
    ...
    times[0] = year
    T2[0] = np.average(T2_daily,axis = 0)
    root.close()
