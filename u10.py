...
for year in range(st_year,ed_year,1):
    ...
    U10 = root.createVariable('U10',np.float32,('time','lat','lon')) ####################################
    #SWDOWN,GLW
    # 对nc文件进行描述
    ...
    #                print(dfile)
            ...
            U10[count:count+len(dfiles)-1] = data_reinit_sn.U10[1:len(dfiles)].values #####################################
        else :
            raise NameError
        count += 24
    root.close()
    ...
    U10[:len(dates)] = np.average(U10_hourly,axis = 1)   ###################################
    ...
    for i in range(12): 
        times[i] = i+1
        U10[i] = np.average(U10_daily[index_month[i]:index_month[i+1]],axis = 0)   ###################################
    root.close()
    ...
    times[0] = year
    U10[0] = np.average(U10_daily,axis = 0)   ##################################
    root.close()
